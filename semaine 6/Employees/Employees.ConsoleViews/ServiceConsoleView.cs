...
namespace Employees.ConsoleViews
{
	public class ServiceConsoleView : ConsoleView
	{
		...
		public override ConsoleKeyInfo display()
		{
            ...
            ServiceControler.Instance.subscribe(() =>
            {
				   ...
				   (ref ConsoleKeyInfo cki) =>
				   {
					   ...
					   switch (cki.KeyChar.toUpper())
					   {
                            case 'A':
                                try
                                {
						            ServiceControler.Instance.addService(
						   ...
						   case 'U':
                                ...
                                try
                                {
                                    Service targetService = ServiceControler.Instance.Services.ElementAt(r - 1);
                                    ServiceControler.Instance.updateService(targetService,
                                    this.input<string>(
                                        (string s) => { 
                            ...
                            case 'D':
							   ...
							   try
							   {
								   ServiceControler.Instance.deleteService<Service>(ServiceControler.Instance.Services.ElementAt(r - 1));
                               }
							   catch (Exception e)
							   {
								   throw e;
		...
		private void menu()
		{
			...
			int i = 0, padleft = ServiceControler.Instance.Services.Count().ToString().Length;
			foreach (Service service in ServiceControler.Instance.Services)
				Console.WriteLine((++i).ToString().PadLeft(padleft) + " - " + service);
			Console.WriteLine("-------------");
			Console.WriteLine("[A]dd a service " + (ServiceControler.Instance.Services.Count() > 0 ? "[U]pdate a service [D]elete a service [S]earch " : "") + "[ESC]quit");
		}
	}
}
