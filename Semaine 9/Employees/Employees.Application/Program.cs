...
namespace Employees.Application
{
    static class Program
    {
        ...
        static void runServiceView()
        {
            ...
            try
            {
                ServiceController.Instance.updateService(ServiceController.Instance.Services.ElementAt(0), "XXXXX", "");
            }
            catch (Exception e)
            {
            }
            Console.WriteLine("--------------------------------------------------------------");
            Console.WriteLine(ServiceController.Instance.retrieveService("BUYER",ServiceSearchOption.SearchOnCode));

            ServiceController.addServiceDelegate d = (Service s) => {
                    Console.WriteLine(s);
                };
            Service s2 = ServiceController.Instance.addService("XXXXX", "THE X SERVICE",
                d
            );
            ...
            ServiceController.Instance.updateService(s2,"XXXX1","The X SERVICE");
