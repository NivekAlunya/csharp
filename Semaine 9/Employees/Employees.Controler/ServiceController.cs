    ...
    public class ServiceController
    {
        ...
        public List<Service> Services 
        {
            get
            {
                if (null == _services)
                {
                    _services = ServiceStore.Instance.getServices();
                    this.sort();
                }
                return _services;
            }
        ...
        public Service addService(string code, string label,addServiceDelegate fn = null)
        {
            ...
            try
            {
                service = ServiceStore.Instance.addService(code, label);
            }
            catch (Exception e)
            {
                throw e;
        ...
        public void updateService(Service service,string code,string label)
        {
            ...
            ServiceStore.Instance.updateService(service,code,label);
        ...
        public bool deleteService<T>(T target,ServiceSearchOption option = ServiceSearchOption.SearchOnService)
        {
            ...
            if (null != targetService)
            {
                ...
                return ServiceStore.Instance.deleteService(targetService);
            }
            return false;
        }

        public IEnumerable<Employee> retrieveEmployees(Service service)
        {
            var linq = from Employee emp in EmployeeStore.Instance.getEmployees()
                       where emp.Service.Code == ( null != service.Code ? service.Code : null )
                       select emp //new { fullname = emp.Surname + " " + emp.Firstname }
            ;
            return new List<Employee>(linq);
        }
        private void sort()
        {
            this.Services.Sort(
